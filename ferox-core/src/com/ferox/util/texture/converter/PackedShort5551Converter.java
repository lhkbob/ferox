...
public class PackedShort5551Converter implements Decoder, Encoder {
	...
	public boolean canEncode(DataType type, TextureFormat format) {
		return type == DataType.UNSIGNED_SHORT && (format == TextureFormat.ABGR_1555 || 
			   format == TextureFormat.BGRA_5551 || format == TextureFormat.ARGB_1555 || 
	...
	public void getColor(DataBlock data, float u, float v, float w, Color4f store) {
		...
		case ABGR_1555:
			store.set(((val & C4_REV_MASK) >> 0) / MAX_VALUE, ((val & C3_REV_MASK) >> 5) / MAX_VALUE, 
					  ((val & C2_REV_MASK) >> 10) / MAX_VALUE, (val & C1_REV_MASK) != 0 ? 1f : 0f);
			break;
		case ARGB_1555:
			store.set(((val & C2_REV_MASK) >> 10) / MAX_VALUE, ((val & C3_REV_MASK) >> 5) / MAX_VALUE,
					  ((val & C4_REV_MASK) >> 0) / MAX_VALUE, (val & C1_REV_MASK) != 0 ? 1f : 0f);
			break;
		case BGRA_5551:
			store.set(((val & C3_MASK) >> 1) / MAX_VALUE, ((val & C2_MASK) >> 6) / MAX_VALUE, 
					 ((val & C1_MASK) >> 11) / MAX_VALUE, (val & C4_MASK) != 0 ? 1f : 0f);
			break;
		case RGBA_5551:
			store.set(((val & C1_MASK) >> 11) / MAX_VALUE, ((val & C2_MASK) >> 6) / MAX_VALUE, 
					  ((val & C3_MASK) >> 1) / MAX_VALUE, (val & C4_MASK) != 0 ? 1f : 0f);
			break;
		}
	}
