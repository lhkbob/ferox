...
import com.ferox.util.geom.GeometryDirtyState;
import com.ferox.util.geom.Geometry.CompileType;
import com.ferox.util.geom.GeometryDirtyState.BufferRange;
    ...
    private void updateForVbo(Geometry g, GeometryHandle handle, GeometryDirtyState dirty) {
        ...
        if (dirty != null) {
            ...
            for (int i = vas.size() - 1; i >= 0; i--) {
                ...
                VertexAttribute vb = g.getAttribute(va.name);
            ...
            for (int i = newBuffersReq.size() - 1; i >= 0; i--) {
                VertexAttribute vb = g.getAttribute(newBuffersReq.get(i));
            ...
            if (!fullAllocate) {
                for (Entry<String, VertexAttribute> attr: g.getAttributes().entrySet()) {
                    String name = attr.getKey();
                    VertexAttribute vb = attr.getValue();
...
//        }
    ...
    private boolean checkVertexArraySize(Geometry g) {
        int elementCount = -1;
        for (VertexAttribute vb: g.getAttributes().values()) {
