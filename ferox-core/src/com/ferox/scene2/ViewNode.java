 ...
 * When it is true, a {@link FrustumController} will update the frustum to match
 ...
 * {@link SceneElement}s and ViewNodes will by updated by a FrustumController so
 * that the frustum's orientation matches the transformation of the Entity's
 * SceneElement.
 * </p>
 * 
 * @see FrustumController
...
public class ViewNode extends Component {
	...
	public ViewNode(RenderSurface surface, float fov, float znear, float zfar) {
		...
		frustum = new Frustum(fov, surface.getWidth() / surface.getHeight(), znear, zfar);
	}
	 ...
	 * FrustumController may overwrite changes to the Frustum's orientation.
	...
	/**
	 * <p>
	 * Return whether or not a {@link FrustumController} should update the
	 ...
	 * {@link FrustumController} to reflect changes in its linked
