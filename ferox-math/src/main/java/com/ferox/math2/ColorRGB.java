...
/**
 * <p>
 * ColorRGB is a 3-tuple that stores red, green, and blue color components to
 * represent a color in the classical RGB scheme. It does not support sRGB,
 * although there is nothing stopping you from storing sRGB within the color and
 * performing the math manually.
 * </p>
 * <p>
 * Color component values are normally defined in the range from 0 to 1, where 1
 * represents full saturation of the component. So (0, 0, 0) is black and (1, 1,
 * 1) would be white. ColorRGB stores color values past 1 so that high-dynamic
 * range colors can be used. However, ReadOnlyColor3f does not perform any
 * tone-mapping so (2, 2, 2) and (3, 1, 5) would both be considered as (1, 1, 1)
 * when asking for LDR values.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * It is a common practice to use a {@link Vector4} when an RGBA color is
 * needed, such as in the lower-level rendering APIs in com.ferox.renderer.
 * </p>
 * 
...
public final class ColorRGB {
    ...
    /**
     * Create a new ColorRGB that copies the color values from <tt>color</tt>
     * initially.
     * 
    ...
    /**
     * Compute a brighter version of <tt>color</tt>, adjusted by the given
     * brightness <tt>factor</tt> and store it in this color. The
     * <tt>factor</tt> represents the fraction to scale each component by. This
     * uses the HDR color values, so the brighter color might extend into high
     * dynamic range values (i.e. if you brightened white).
     * 
    ...
    /**
     * Convenience function to darken this color. This is equivalent to
     * <code>darker(this, 0.7);</code>
     * 
     * @return This color
     */
    ...
    /**
     * Compute a darker version of <tt>color</tt>. The <tt>factor</tt>
     * represents the fraction to scale each component by. This uses the HDR
     * color values, so it might be possible for the darkened color to be equal
     * in the low dynamic range.
     * 
    ...
    /**
     * As {@link #set(double[], int)} but a DoubleBuffer is used as a source for
     * float values.
     * 
    ...
    /**
     * As {@link #getHDR(double[], int)} except the values are cast to floats to
     * store in the array.
     * 
     * @param vals
     * @param offset
