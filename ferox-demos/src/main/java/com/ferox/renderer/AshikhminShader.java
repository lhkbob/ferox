...
public class AshikhminShader implements Task<Void> {
    ...
    private volatile double texCoordScale;
    ...
    public AshikhminShader(Framework f) throws Exception {
        ...
        input.on(or(mouseRelease(MouseEvent.MouseButton.LEFT), mouseRelease(MouseEvent.MouseButton.RIGHT))).trigger(new Action() {
                 @Override
                 public void perform(InputState prev, InputState next) {
                     moveState = MoveState.NONE;
                 }
        ...
        final JSlider fullSlider = createSlider();
        final JSlider normalSlider = createSlider();
        final JSlider specAlbedoSlider = createSlider();
        final JSlider diffAlbedoSlider = createSlider();
        final JSlider shininessSlider = createSlider();
        ...
        JLabel tcVLabel = new JLabel("TC Scale");
        final JSlider tcVSlider = createSlider(100, 10000);
        tcVSlider.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                texCoordScale = tcVSlider.getValue() / 100.0;
            }
        });

        layout.setHorizontalGroup(layout.createSequentialGroup()
                                        .addGroup(layout.createParallelGroup().addComponent(loadTexturesA)
                                                        .addComponent(loadTexturesB).addComponent(fullSlider)
                                                        ...
                                                        .addComponent(tcVSlider))
                                        .addGroup(layout.createParallelGroup().addComponent(texLabelA)
                                                        .addComponent(texLabelB).addComponent(fullLabel)
        ...
        layout.setVerticalGroup(layout.createSequentialGroup()
                                      ...
                                      .addGroup(layout.createParallelGroup().addComponent(tcVSlider)
                                                      .addComponent(tcVLabel)));
    ...
    public Void run(HardwareAccessLayer access) {
        ...
        r.setUniform(shader.getUniform("uLightPos"), new Vector4().mul(camera.getViewMatrix(), light));
        ...
        r.setUniform(shader.getUniform("uTCScale"), texCoordScale, texCoordScale);

        Matrix4 mv = new Matrix4();
        mv.mul(camera.getViewMatrix(), modelTrackBall.getTransform());

        r.setUniform(shader.getUniform("uModelview"), mv);
        r.setUniform(shader.getUniform("uInverseModelview"), mv.inverse());
