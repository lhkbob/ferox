...
vec3 lookupSpecular(vec3 r, float exp) {
    if (uShininessMax < 0.0) {
        return texture(uIrradianceMax, uInvView * r).rgb;
    } else {
        float alpha = (exp - uShininessMin) / (uShininessMax - uShininessMin);
        return mix(texture(uIrradianceMin, uInvView * r).rgb, texture(uIrradianceMax, uInvView * r).rgb, alpha);
    }
}
...
void main() {
    ...
    if (uDiffuseMode) {
        ...
        diff = viewTerm * diff * (1.0 - spec) * texture(uIrradianceMin, uInvView * fN).rgb;
