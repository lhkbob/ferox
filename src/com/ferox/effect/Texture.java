 ...
 * Defaults:
 * 
 * COMBINE_RGB = CombineRgb.MODULATE; COMBINE_ALPHA = CombineAlpha.MODULATE;
 * 
 * SOURCE_RGB0 = CombineSource.PREV_TEX; SOURCE_RGB1 = CombineSource.CURR_TEX;
 * SOURCE_RGB2 = CombineSource.VERTEX_COLOR; SOURCE_ALPHA0 =
 * CombineSource.PREV_TEX; SOURCE_ALPHA1 = CombineSource.CURR_TEX; SOURCE_ALPHA2
 * = CombineSource.VERTEX_COLOR;
 * 
 * OP_RGB0 = CombineOperand.COLOR; OP_RGB1 = CombineOperand.COLOR; OP_RGB2 =
 * CombineOperand.ALPHA; OP_ALPHA0 = CombineOperand.ALPHA; OP_ALPHA1 =
 * CombineOperand.ALPHA; OP_ALPHA2 = CombineOperand.ALPHA;
 * 
 * ENV_MODE = EnvMode.MODULATE; COORD_GEN = TexCoordGen.NONE;
 * 
...
public class Texture extends AbstractEffect {
	...
	/**
	 * How a given texture coordinate is found. NONE uses none, or the actual
	 * texture coordinates of the rendered object. OBJECT uses the texture coord
	 * generation planes in object space to generate coords. EYE uses those
	 * planes similarly to object space, but after everything has moved to eye
	 * space. SPHERE sets up coordinates suitable to access a sphere mapped
	 * texture image. REFLECTION generates reflection vector coordinate at a
	 * given location (vertex normal + viewer location, etc) NORMAL generates
	 * vector coordinates based off of the surface normal.
	 * 
	 * OBJECT, EYE can work with any type of texture, and any of STR
	 * coordinates. SPHERE is intended for ST coordinates with a 2D texture.
	 ...
	 * operation. CURR_TEX = color of this texture image PREV_TEX = color of the
	 * texture in the texture unit processed just before this one (if this is
	 * the first unit, then it's the same as VERTEX_COLOR). BLEND_COLOR =
	 * environment color of this texture VERTEX_COLOR = color computed based on
	 * material color and lighting TEXi = color of the texture image bound to
	 * the given unit (note not all TEXi will be supported because hardware may
	 * not have that many texture units available. Units beyond 8 are included
	 * for advanced graphics cards (or future cards)).
	 */
	public static enum CombineSource {
		CURR_TEX, PREV_TEX, BLEND_COLOR, VERTEX_COLOR, TEX0, TEX1, TEX2, TEX3, TEX4, TEX5, TEX6, TEX7, TEX8, TEX9, TEX10, TEX11, TEX12, TEX13, TEX14, TEX15, TEX16, TEX17, TEX18, TEX19, TEX20, TEX21, TEX22, TEX23, TEX24, TEX25, TEX26, TEX27, TEX28, TEX29, TEX30, TEX31
	}
	 ...
	 * 
	 * SPHERE mapping isn't supported on the 3rd coordinate, so if SPHERE is
	 * given, NONE is used instead.
	 * 
