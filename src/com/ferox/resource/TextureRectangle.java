...
/** Represents a rectangular image with different access semantics.
 * The texture data is accessed by unnormalized texture coordinates, from
 * 0 to width and 0 to height, which is very different for most other texture
 * coordinate access that uses a 0 to 1 scale.
 * 
 * TextureRectangles do not support mipmaps, and can only have the wrap mode
 * of CLAMP used for s and t coordinates.  Given this, if a filter other than
 * LINEAR or NEAREST is used, it defaults to the appropriate non-mipped version.
 * setWrapX() is overridden to set it to CLAMP.
 * 
 * TextureRectangles are not supported on all hardware, but because texture
 * coordinate access is different, there is no automatic fallback that a Renderer
 * can use.  Support should be checked before texture rectangles are used,
 * otherwise updates may result in a status of ERROR.
 * 
 * Besides the arguments present in TextureImage's constructors,
 * TextureRectangle adds the additional parameters BufferData data and 
 * int width/height.  width and height represents the pixel dimension of the sides
 * of the image.  data is the primitive data for one image.  TextureRectangles do
 * not support mipmaps.
 * 
 ...
 * 2. data must be sized correctly based on the layer's dimension, format and type.
 * 3. data must have the same data type as passed into the constructor.
 * 
 * TextureRectangle provides methods to mark regions of the texture's image
 * as dirty.  These commands will always be clamped to the
...
public class TextureRectangle extends TextureImage {
	 ...
	 * getDirtyDescriptor() for texture rectangle's will return objects of this class. */
	public static class TextureRectangleDirtyDescriptor extends TextureDirtyDescriptor {
		 ...
		 * 
		 * The returned region will be constrained to be in the dimensions of the texture. */
	...
	public TextureRectangle(BufferData data, int width, int height, TextureFormat format, DataType type) throws NullPointerException, IllegalArgumentException {
		super(format, type);
	...
	public TextureRectangle(BufferData data, int width, int height, TextureFormat format,  DataType type, Filter filter) throws StateException {
	...
	public TextureRectangle(BufferData data, int width, int height, TextureFormat format, DataType type, Filter filter, TextureWrap wrapAll, DepthMode depthMode, PixelTest depthTest) throws StateException {
	...
	private void setData(BufferData data, int width, int height) throws IllegalArgumentException {
		...
		if (data != null) {
			if (data.getType() != type)
				throw new IllegalArgumentException("BufferData doesn't have a matching type for the texture, expected: " + type + ", but was: " + data.getType());
			if (data.getCapacity() != format.getBufferSize(width, height, 1))
				throw new IllegalArgumentException("Buffer does not have the correct size, expected: " + format.getBufferSize(width, height, 1) + ", but was: " + data.getCapacity());
		} else {
			if (format.isCompressed())
				throw new StateException("Headless TextureRectangle cannot have a client compressed texture: " + format);
		}
	...
	/** Mark a region of the texture image as dirty.
	 * 
	 * The x, y offsets and width and height will be clamped to be
	 * within the valid region of the given layer. */
	public void markDirty(int x, int y, int width, int height) {
		TextureRectangleDirtyDescriptor dirty = (TextureRectangleDirtyDescriptor) this.getDirtyDescriptor();
		
		if (dirty.dirtyRegions == null)
			dirty.dirtyRegions = new MipmapDirtyRegion(x, y, 0, width, height, 0,
	...
	/** Get the buffer data for the texture rectangle (there are no
	 * other mipmaps).
	 * 
