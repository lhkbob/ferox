...
/** A UnitList is meant to store the list of <unit, item>
 * intersection, often when using textures.
 * 
 * The UnitList supports units >= 0, with no maximum limit.
 * It will dynamically change between array access and list
 * access to maximize storage size and search time.
 * 
...
public class UnitList<T> {
	/** Represents a single intersection between the unit value
	 * and the item at that unit. */
	...
	private List<Unit<T>> readOnlyUnits;
	...
	/** Set the item on the given unit.  If item is null,
	 * makes it so that there is no item bound on the given unit.
	 * 
	 * If unit is less than 0, then this method fails. */
	@SuppressWarnings("unchecked")
	public void setItem(int unit, T item) throws IllegalArgumentException {
		if (unit < 0)
			throw new IllegalArgumentException("Invalid unit, units cannot be less than 0: " + unit);
		...
		if (this.units.isEmpty()) { // no more textures left, so reset the ra record
			this.resetRandomAccess();
			return;
		}
		...
		if (!useRa) {
			...
			if (this.useRandomAccess) {
				if (minUnit != this.raUnitOffset || maxUnit != (this.raUnitOffset + this.randomAccessData.length - 1)) {
					...
					System.arraycopy(this.randomAccessData, Math.max(0, minUnit - this.raUnitOffset), newTex, Math.max(0, this.raUnitOffset - minUnit),
	...
	/** Get the item currently set for the given unit. 
	 * A return value of null signifies that there is no item
	 * bound to that unit.
	 * 
	...
	/** Return a list of all bounds units.  Unit values not
	 * present have no item bound to them. 
	 * 
