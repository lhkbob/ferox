...
public class DDSTexture {
	...
	private void readData(InputStream in) throws IOException {
		...
		this.data = new BufferData[arrayCount][this.mipmapCount];
		byte[] image;
		for (int i = 0; i < arrayCount; i++) {
			...
			for (int m = 0; m < this.mipmapCount; m++) {
				size = this.format.getBufferSize(width, height, depth) * this.type.getByteSize();

				image = new byte[size];
				readAll(in, image);
				this.data[i][m] = this.createBuffer(image);
	...
	private BufferData createBuffer(byte[] image) throws IOException {
		...
		throw new IOException("Unsupported opengl texture data type: " + this.type);
	}
