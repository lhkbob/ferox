 ...
 * use.
 * 
 * Also, transforms and clip planes are not tracked because that is more
 * complicated, and the tracking would somewhat defeat the purpose of doing that
 * math on the graphics card.
 * 
 * Because transform's are not tracked, drivers that modify the matrix mode must
 * be sure to change it back to GL_MODELVIEW, or it will break the functionality
 * of JoglTransformDriver.
 * 
...
public class JoglStateRecord {
	...
	public JoglStateRecord(RenderCapabilities caps) {
		...
		int maxTU = Math.max(caps.getMaxFixedPipelineTextures(), Math.max(caps
				.getMaxVertexShaderTextures(), caps
				.getMaxFragmentShaderTextures()));
		textureRecord = new TextureRecord(maxTU);
		vertexArrayRecord = new VertexArrayRecord(caps
				.getMaxTextureCoordinates(), caps.getMaxVertexAttributes());
	}
}
