 ...
 * not override those methods for customizability.  Instead, use the onX() methods
 * to achieve that.  Also, any subclasses that use a GLAutoDrawable must be sure
 * to add themselves as a GLEventListener to function properly.
 * 
 * @author Michael Ludwig
 */
public abstract class JoglRenderSurface extends AbstractRenderSurface implements ContextRecordSurface, AttachableSurfaceGLEventListener {
	...
	/** Return the GLAutoDrawable associated with this surface.  Return
	 * null if there is no GLAutoDrawable for the surface (at the moment
	 * this only applies to JoglTextureSurfaces using an FboDelegate).
	 * 
	 * If null is returned, the surface will be attached to the shadow
	 * context or another surface to be rendered during their drawable's
	 * display() method.
	 * 
	 * When null is not returned, the drawable will have its display()
	 * method executed during the factory's renderFrame() method. */
	public abstract GLAutoDrawable getGLAutoDrawable();
	
	/** All JoglRenderSurfaces must use a JoglStateRecord. 
	 * Returns the JoglStateRecord that's associated with the
	 * drawable from getGLAutoDrawable().  If this is null,
	 * the effective state record for the surface is the state
	 * record of the current surface. */
	public abstract JoglStateRecord getStateRecord();
	
	/** Called by this surface's factory its destroy(surface)
	 * method is called.  The surface's context will not be current, so
	...
	/** Called after the JoglSurfaceFactory has been notified that this
	 * is the active surface, and after resources have been potentially
	 * managed, but before the surface has been cleared, and before the
	 * render action has been executed. */
	public abstract void preRenderAction();
	
	/** Called right after the render action has completed, while their
	 * is still a valid gl context active.
	 * 
	...
	/** Called before preRenderAction() the first time this
	 * surface will be rendered.  There is a suitable gl available
	 * from the factory. */
	...
	public void assignResourceAction(Runnable runnable) {
		this.helper.assignResourceAction(runnable);
	}
	...
	public void render() throws RenderException {
		GLAutoDrawable gd = this.getGLAutoDrawable();
		if (gd == null)
	...
	/** Utility method to run this surfaces renderAction and call all
	 * of its callbacks: init(), preRenderAction(), and postRenderAction().
	 * 
	 * This must only be called from within the display() method of an
	...
	public void displaySurface(JoglRenderSurface next) {
		...
		this.postRenderAction(next);
	}
	
	private void clearBuffers() {
		FramebufferRecord fr = this.getStateRecord().frameRecord;
		PixelOpRecord pr = this.getStateRecord().pixelOpRecord;
		...
		Color clearColor = this.getClearColor();
		if (!clearColor.equals(fr.clearColor)) {
			...
			clearColor.get(fr.clearColor);
		}
		float clearDepth = this.getClearDepth();
		if (fr.clearDepth != clearDepth) {
			gl.glClearDepth(clearDepth);
			fr.clearDepth = clearDepth;
		}
		int clearStencil = this.getClearStencil();
		...
		int clearBits = 0;
		if (this.isColorBufferCleared())
			clearBits |= GL.GL_COLOR_BUFFER_BIT;
		if (this.isDepthBufferCleared())
			clearBits |= GL.GL_DEPTH_BUFFER_BIT;
		if (this.isStencilBufferCleared())
