...
/**
 * JoglVertexBufferGeometryDriver handles the rendering of
 * VertexBufferGeometries. It is somewhat liberal in its assumptions about the
 * state record. To be efficient, it assumes that no state driver modifies the
 * VertexArrayRecord and that other geometry drivers properly reset the state
...
public class JoglVertexBufferGeometryDriver implements GeometryDriver {
	...
	private final BufferedGeometryDriver<VertexBufferObject, VertexBufferGeometry> geomDriver;
	...
	public JoglVertexBufferGeometryDriver(JoglContextManager factory) {
		this.factory = factory;
		geomDriver = new BufferedGeometryDriver<VertexBufferObject, VertexBufferGeometry>();
	...
	public int render(Geometry geom, ResourceData data) {
		...
		AbstractRenderer renderer = factory.getRenderer();

		BufferedGeometryHandle<VertexBufferObject> toRender = (BufferedGeometryHandle<VertexBufferObject>) data
				.getHandle();
		...
		if (toRender != lastRendered) {
			...
			boolean abort = !(toRender.glInterleavedType < 0 ? setVertexPointers(
					gl, renderer, vr, toRender)
			...
			if (abort) {
				...
				data
						.setStatusMessage("VertexBufferObjects used by this geometry have been cleaned-up");
		...
		if (elementVbo == null) {
			...
			data.setStatus(Status.ERROR);
			data
					.setStatusMessage("VertexBufferObject used for indices has been cleaned-up");
			return 0;
		} else {
			...
			if (elementVbo.getId() != vr.elementBufferBinding) {
				...
				gl.glBindBufferARB(GL.GL_ELEMENT_ARRAY_BUFFER_ARB,
						vr.elementBufferBinding);
			}

			DataType type = indices.getType();
			int byteOffset = indices.getAccessor().getOffset()
					* type.getByteSize();
			gl.glDrawRangeElements(toRender.glPolyType, 0,
					toRender.vertexCount, indices.getElementCount(), JoglUtil
							.getGLType(type), byteOffset);
	...
	public void cleanUp(Resource resource, ResourceData data) {
		// do nothing -> nothing to cleanup since the vbos are cleaned up
		// separately
	}
	...
	public void update(Resource resource, ResourceData data, boolean fullUpdate) {
		BufferedGeometryHandle<VertexBufferObject> handle = (BufferedGeometryHandle<VertexBufferObject>) data
				.getHandle();
		handle = geomDriver.updateHandle((VertexBufferGeometry) resource,
				handle, maxTextureUnits, maxVertexAttribs);
		...
		} else if (!getVboStatusValid(factory.getRenderer(), handle)) {
			// this correctly checks for vbo support
			data.setStatus(Status.ERROR);
			data
					.setStatusMessage("At least one VertexBufferObject in use have a status of CLEANED or ERROR");
		} else {
	 ...
	 */
	private boolean getVboStatusValid(Renderer renderer,
			BufferedGeometryHandle<VertexBufferObject> handle) {
		Status s;

		// check all vertex vbos
		int count = handle.compiledPointers.size();
		for (int i = 0; i < count; i++) {
			s = renderer.getStatus(handle.compiledPointers.get(i).array
					.getArray());
	 ...
	 * next.
	 * 
	 * This works even if the last used handle was aborted because it referenced
	 * an invalid vbo, since we don't do any vbo binding here.
	 */
	private static void disablePointers(GL gl, VertexArrayRecord vr,
			BufferedGeometryHandle<VertexBufferObject> lastUsed,
			BufferedGeometryHandle<VertexBufferObject> next) {
	 ...
	 * 
	 * As below, this modifies the array buffer binding, and returns true if
	 * there were no problems about the vbos.
	 */
	private static boolean setInterleavedArray(GL gl,
			AbstractRenderer renderer, VertexArrayRecord vr,
			BufferedGeometryHandle<VertexBufferObject> toRender) {
		Handle vbo = renderer.getHandle(toRender.compiledPointers.get(0).array
				.getArray());
		if (vbo == null)
			return false; // abort
	 ...
	 * in the reset() method.
	 * 
	 * Returns true if all vbos were bound successfully, or false if the
	 * renderer returned a null handle and the rendering should be aborted.
	 */
	private static boolean setVertexPointers(GL gl, AbstractRenderer renderer,
			VertexArrayRecord vr,
			BufferedGeometryHandle<VertexBufferObject> toRender) {
		ArrayPointer<VertexBufferObject> ap;
		...
		for (int i = 0; i < count; i++) {
			...
			case VERTEX_ATTRIBUTES:
				...
				gl.glVertexAttribPointer(ap.unit, elementSize, JoglUtil
						.getGLType(type), false, byteStride, byteOffset);
				break;
			}
		}
