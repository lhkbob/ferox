 ...
 * one active state.  Each of these units have no effective change on the output
 * if the states per unit were somehow re-ordered (e.g. a light on unit 0 has the
 * same effect as if it were on unit 1).  Multi-texturing is an example of 
 * a type with multiple units, but where the order actually matters, and hence is
 * not a MultiStateDriver.
 * 
 * A MultiStateDriver assumes that its type are described by 1 type (or subclasses
 * of that class type).
 * 
 * @author Michael Ludwig 
 * 
 */
public abstract class MultiStateDriver<T extends State> implements StateDriver {
	/* Simple class to hold the union of a state and its influence/priority. */
	private static class Unit<T extends State> {
		T state;
		float priority;
	}
	...
	private Unit<T>[] queue; // array, maintained in decreasing order by priority
	private int queueSize;
	
	private Unit<T>[] apply; // stores the last-applied and next states (last-applied has priority < 0).
	
	private T dfltState;
	protected final JoglSurfaceFactory factory;
	private Class<? extends State> stateType;
	...
	protected MultiStateDriver(T dfltState, Class<T> stateType, int maxUnits, JoglSurfaceFactory factory) {
		...
		if (this.queueSize > 0) {
			...
			for (i = 0; i < this.queueSize; i++) {
				...
				if (q.state != null) {
					...
					for (j = 0; j < maxUnits; j++) {
						o = this.apply[j];
						if (o.priority == INVALID_PRIORTY) { // only choose unit that hasn't been updated yet
	...
	public void queueAppearanceState(State state) throws RenderException {
		// acceptable, as in SingleStateDriver
	...
	public void queueInfluenceState(State state, float influence) throws RenderException {
		if (!this.stateType.isInstance(state))
			throw new RenderException("Unsupported state type: " + state + ", expected an instance of " + this.stateType);
		...
		for (int i = 0; i < this.queueSize; i++) {
			...
			if (influence >= u.priority) {
				...
				this.queueSize = Math.min(this.queueSize + 1, this.queue.length);
