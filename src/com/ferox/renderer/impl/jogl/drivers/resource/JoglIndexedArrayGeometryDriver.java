...
public class JoglIndexedArrayGeometryDriver implements GeometryDriver {
	...
	private boolean updateArrayVboData(GL gl, VertexArrayRecord var,
		...
		// restore old binding
		gl.glBindBufferARB(GL.GL_ARRAY_BUFFER_ARB, var.arrayBufferBinding);
		return true;
	}
	...
	private boolean updateElementVboData(GL gl, VertexArrayRecord var,
		...
		gl.glBindBufferARB(GL.GL_ELEMENT_ARRAY_BUFFER_ARB, handle.elementVbo);
	...
	private void bindVbos(GL gl, VertexArrayRecord vr, int arrayId,
		...
		if (vr.elementBufferBinding != elementId) {
			...
			gl.glBindBufferARB(GL.GL_ELEMENT_ARRAY_BUFFER_ARB, elementId);
		}
	}
