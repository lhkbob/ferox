...
public class TextureImageDriver {
	// extensions present
	private final boolean npotSupported; // whether or not npot's are supported
	// - according to constructor
	// factory
	private final boolean floatSupported; // whether or not unclamped floating
	// formats are okay
	...
	/**
	 * Utility method to configure the unpack state of the given gl so that the
	 * next glTexImage or glTexSubImage will load data from the given sub-block
	...
	/**
	 * This is a utility method to create a valid TextureHandle object. It
	...
	public TextureHandle createNewTexture(GL gl, TextureImage ti) {
		...
		// pick glTarget
		int glTarget = JoglUtil.getGLTextureTarget(ti.getTarget());
	...
	/**
	 * Determines if the handle is dirty - e.g. it's format and type differ or
	...
	/**
	 * Return a string dirty message that explains why isDirty() would return
	 * true.
	 */
	...
	/**
	 * Wrap the given data's array in a Buffer of the appropriate type. Assumes
	 * that the data and its array are not null.
	 */
