 ...
 * 
 * A Texture2D will only have an ERROR status if it is compressed and
...
public class JoglTexture2DResourceDriver implements ResourceDriver {
	...
	public JoglTexture2DResourceDriver(JoglSurfaceFactory factory) {
	...
	public void update(Resource resource, ResourceData data, boolean fullUpdate) {
		...
		if (data.getStatus() != Status.ERROR) {
			...
			if (handle == null) {
				// make a new texture
				if (t2d.getFormat().isCompressed() && !this.hasS3tcCompression) {
				...
				handle = this.imageDriver.createNewTexture(gl, t2d);
				
				data.setHandle(handle);
				if (this.imageDriver.isDirty(t2d, handle)) {
					data.setStatus(Status.DIRTY);
					data.setStatusMessage(this.imageDriver.getDirtyStatusMessage(t2d, handle));
				} else {
			...
			Texture2DDirtyDescriptor dirty = (Texture2DDirtyDescriptor) t2d.getDirtyDescriptor();
			if (newTex || fullUpdate || dirty.areMipmapsDirty() || dirty.isAnisotropicFilteringDirty() 
				...
				this.imageDriver.setTextureParameters(gl, handle, t2d, newTex || fullUpdate);
				...
				if (newTex || rescale || f.isCompressed() || f == TextureFormat.DEPTH) {
					...
					this.doTexImage(gl, pr, handle, t2d, newTex);
				} else {
					// we can use glTexSubImage for better performance
					this.doTexSubImage(gl, pr, (fullUpdate ? null : dirty), handle, t2d);
				}
	...
	private void doTexImage(GL gl, PackUnpackRecord pr, TextureHandle handle, Texture2D tex, boolean newTex) {
			...
			if (bd != null && bd.getData() != null) {
				if (needsResize) {
					...
					bd = TextureConverter.convert(// src
							                      bd, tex.getFormat(), 
							                      tex.getWidth(i), tex.getHeight(i), 1, 
							                      // dst
				...
				if (handle.glSrcFormat > 0)
					gl.glTexImage2D(handle.glTarget, i, handle.glDstFormat, w, h, 0, handle.glSrcFormat, handle.glType, this.imageDriver.wrap(bd));
				else
					gl.glCompressedTexImage2D(handle.glTarget, i, handle.glDstFormat, w, h, 0, bd.getCapacity(), this.imageDriver.wrap(bd));
			} else if (newTex) {
				// we'll just allocate an empty image
				if (handle.glSrcFormat > 0)
					gl.glTexImage2D(handle.glTarget, i, handle.glDstFormat, w, h, 0, handle.glSrcFormat, handle.glType, null);
				else
					gl.glCompressedTexImage2D(handle.glTarget, i, handle.glDstFormat, w, h, 0, tex.getFormat().getBufferSize(w, h, 0), null);
	...
	private void doTexSubImage(GL gl, PackUnpackRecord pr, Texture2DDirtyDescriptor dirty, TextureHandle handle, Texture2D tex) {
		int w, h;
			...
			if (bd != null && bd.getData() != null) {
				if (dirty == null || dirty.isDataDirty(i)) {
					...
					if (mdr != null) {
						...
						this.imageDriver.setUnpackRegion(gl, pr, mdr.getDirtyXOffset(), mdr.getDirtyYOffset(), 0, w, h);
						gl.glTexSubImage2D(handle.glTarget, i, 
						...
						this.imageDriver.setUnpackRegion(gl, pr, 0, 0, 0, w, h);
						gl.glTexSubImage2D(handle.glTarget, i, 0, 0, w, h, handle.glSrcFormat, handle.glType, this.imageDriver.wrap(bd));
					}
				}
