 ...
 * operations to allocate, update and clean-up vbos on the graphics card. It is
 * based off of the VertexBufferObject resource class. At the moment, it only
 * assumes targets of ARRAY_BUFFER and ELEMENT_ARRAY_BUFFER. The target is
 * guessed on the signed-ness of the type: unsigned types are assumed to be
 * element values.
 * 
...
public class JoglVertexBufferObjectResourceDriver implements ResourceDriver {
	...
	private final boolean vboSupport;

	public JoglVertexBufferObjectResourceDriver(JoglContextManager factory) {
		this.factory = factory;
		vboSupport = factory.getRenderer().getCapabilities()
	...
	public void update(Resource resource, ResourceData data, boolean fullUpdate) {
		...
		if (handle == null) {
			...
			handle.glUsage = JoglUtil.getGLUsageHint(vbo.getUsageHint());
		...
		if (!handle.valid) {
			gl.glBufferDataARB(handle.glTarget, vbo.getData().getCapacity()
					* vbo.getData().getType().getByteSize(),
			...
			if (e == GL.GL_OUT_OF_MEMORY) {
				...
				data.setStatus(Status.ERROR);
				data
						.setStatusMessage("Out of memory, cannot allocate enough space for vbo");
			} else {
				// first successful alloc - after this we can use
				// glBufferSubData()
			...
			VboDirtyDescriptor dirty = (fullUpdate ? null : vbo
					.getDirtyDescriptor());
			int pos = (dirty == null ? 0 : dirty.getDirtyOffset());
			int len = (dirty == null ? vbo.getData().getCapacity() : dirty
					.getDirtyLength());
			...
			if (nio != null) {
				...
				gl.glBufferSubDataARB(handle.glTarget, pos * byteSize, len
						* byteSize, nio);
			}
		}

		// restore the previous buffer binding
		int prevBinding = (handle.glTarget == GL.GL_ARRAY_BUFFER_ARB ? vr.arrayBufferBinding
	...
	public void cleanUp(Resource resource, ResourceData data) {
		if (data.getHandle() != null)
			...
			deleteVbo(factory.getGL(), data.getHandle().getId());
	}

	/*
	 * Return a nio buffer wrapping bd's array, or null if the array is null.
	 */
