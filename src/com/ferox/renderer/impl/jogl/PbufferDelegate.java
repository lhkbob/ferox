...
public class PbufferDelegate extends TextureSurfaceDelegate {
	...
	public void postRenderAction(JoglRenderSurface next) {
		...
		if (color != null) {
			TextureHandle handle = (TextureHandle) this.factory.getRenderer().getHandle(color);
			gl.glBindTexture(handle.glTarget, handle.id);
			this.copySubImage(gl, handle);
			ct = handle.glTarget;
		}
		if (depth != null) {
			TextureHandle handle = (TextureHandle) this.factory.getRenderer().getHandle(depth);
			gl.glBindTexture(handle.glTarget, handle.id);
			this.copySubImage(gl, handle);
			dt = handle.glTarget;
		}
	...
	/* Copy the buffer into the given TextureHandle.  It assumes the texture was already bound. */
	...
	private static void restoreBindings(GL gl, TextureUnit activeUnit, int colorTarget, int depthTarget) {
		if (colorTarget > 0) {
			if (activeUnit.enabledTarget == colorTarget)
				gl.glBindTexture(colorTarget, activeUnit.texBinding); // restore enabled texture
			else
		...
		if (depthTarget > 0 && colorTarget != depthTarget) {
			if (activeUnit.enabledTarget == depthTarget)
				gl.glBindTexture(depthTarget, activeUnit.texBinding); // restore enabled texture
			else
				gl.glBindTexture(depthTarget, 0); // not really the active unit
		}
	}
	
	private static GLCapabilities chooseCapabilities(DisplayOptions request, boolean useDepthRenderBuffer) {
		GLCapabilities caps = new GLCapabilities();
		
		PixelFormat pf = request.getPixelFormat();
		caps.setPbufferFloatingPointBuffers(pf == PixelFormat.RGB_FLOAT || pf == PixelFormat.RGBA_FLOAT);
		
		// try to update the caps fields
		switch(pf) {
		case NONE:
			caps.setRedBits(0); caps.setGreenBits(0); caps.setBlueBits(0); caps.setAlphaBits(0); break;
		case RGB_16BIT:
		...
		case RGB_FLOAT: case RGBA_FLOAT:
			caps.setRedBits(32); caps.setGreenBits(32); caps.setBlueBits(32); caps.setAlphaBits(pf == PixelFormat.RGB_FLOAT ? 0 : 32); break;
		case RGBA_32BIT:
		...
		case NONE:
			caps.setDepthBits(useDepthRenderBuffer ? 24 : 0); break;
		}
