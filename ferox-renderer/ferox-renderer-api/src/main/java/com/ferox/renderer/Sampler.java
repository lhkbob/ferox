...
/**
 * Sampler represents the high-level texture object in OpenGL.  There are two main
 * varieties of sampler: color samplers, and depth/stencil samplers.  The color sampler
 * types all extend from the {@link Texture} interface. The depth/stencil samplers extend
 * from {@link DepthMap}. This helps encode how each sampler can be used into the type
 * system.
 * <p/>
...
public interface Sampler extends Resource {
    ...
    public static enum WrapMode {
        /**
         * Coordinates are clamped such that no samples read beyond the last pixel at each
         * edge. There is no mirroring, repeating, or interpolation with the border color.
         * Visually this appears as though the edge colors are extruded beyond their
         * limits.
         */
        CLAMP,
        /**
         * Coordinates are clamped such that samples are clamped to the border, and are
         * thus presented as the solid border color. Visually this appears as though the
         * image is bordered by the color.
         */
        CLAMP_TO_BORDER,
        /**
         * Coordinates are mirrored at each edge, so the sampling pattern creates a
         * seamless pattern, e.g. 0 -> 1 -> 0 -> 1, etc.
         */
        MIRROR,
        /**
         * Coordinates are reset to 0 or 1 depending on which edge they cross, so the
         * sampling pattern creates a tiled image, e.g. 0 -> 1, 0 -> 1.
         */
        REPEAT
    }
    ...
    /**
     * RenderTarget encapsulates the internal state needed by a Framework to render into a
     * specific 2D region of the sampler. Depending on the specific type of sampler, it
     * may have multiple render targets. Cube maps have six targets, one for each face of
     * the cube. Texture arrays have a target for each indexed image. 3D textures have a
     * target for each 2D slice. 1D and 2D textures have a single render target.
     */
    ...
    /**
     * Get whether or not the samples from the sampler should be interpolated by the
     * hardware. The exact strategy differs depending on the base format and mipmaps
     * present.  If color data is interpolated, it will be done by bilinear interpolation
     * if no mipmaps are present, or trilinear interpolation if mipmaps exist. Depth
     * values are interpolated in a way to approximate soft shadows.
     *
    ...
    /**
     * Get the height of the sampler at the 0th mipmap level. For one-dimensional
     * textures, this will return 1.
     *
    ...
    /**
     * Get the depth of the sampler at the 0th mipmap level. For one or two dimensional
     * textures, this will return 1.
     *
    ...
    /**
     * Get the WrapMode applied to texture coordinates accessing this sampler. The wrap
     * mode is the same for each coordinate, S, T, R, and Q.
     *
    ...
    /**
     * Get the base or lowest mipmap that contains valid image data. The lowest mipmap
     * corresponds to the mipmap with largest dimensions and most detail. Generally this
     * will be 0, but may be a larger number if the texture contains partially loaded or
     * defined mipmap levels.
     * <p/>
     * Mipmap layers between the base mipmap and the max mipmap will always have image
     * data.
     *
    ...
    /**
     * Get the max or highest mipmap that contains valid image data. The highest mipmap
     * corresponds to the mipmap with the smallest dimensions. Generally this will be
     * equal to the log base 2 of the maximum dimension of the texture, but may be smaller
     * if the texture contains partially defined mipmap levels.
     * <p/>
     * Mipmap layers between the base mipmap and the max mipmap will always have image
     * data.
     *
     ...
     * INT_BIT_FIELD, the components are most likely packed into single primitives.
     * Otherwise, as long as the data isn't compressed, each primitive corresponds to one
     * of the components in the base format.
     * <p/>
     * The combination of base format and data type do not guarantee a unique description
     * of how to interpret the data. The exact interpretation depends on the data
     * assignment method invoked on the builder that created the resource.
     *
