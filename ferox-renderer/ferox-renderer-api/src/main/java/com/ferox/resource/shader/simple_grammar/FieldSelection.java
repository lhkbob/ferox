...
public class FieldSelection extends AbstractLValue {
    private static final Pattern VEC2_ACCESS = Pattern
            .compile("([xy]([xy])?)|([rg]([rg])?)|([st]([st])?)");
    private static final Pattern VEC3_ACCESS = Pattern.compile(
            "([xyz]([xyz])?([xyz])?)|([rgb]([rgb])?([rgb])?)|([stp]([stp])?([stp])?)");
    private static final Pattern VEC4_ACCESS = Pattern.compile(
            "([xyzw]([xyzw])?([xyzw])?([xyzw])?)|([rgba]([rgba])?([rgba])?([rgba])?)|([stpq]([stpq])?([stpq])?([stpq])?)");
        ...
        if (base instanceof Struct) {
            ...
            case VEC2:
                if (VEC2_ACCESS.matcher(field).matches()) {
                    return (field.length() == 1 ? PrimitiveType.FLOAT
                                                : PrimitiveType.VEC2);
                }
                break;
            case IVEC2:
                if (VEC2_ACCESS.matcher(field).matches()) {
                    return (field.length() == 1 ? PrimitiveType.INT
                                                : PrimitiveType.IVEC2);
                }
                break;
            case BVEC2:
                if (VEC2_ACCESS.matcher(field).matches()) {
                    return (field.length() == 1 ? PrimitiveType.BOOL
                                                : PrimitiveType.BVEC2);
                }
                break;
            case VEC3:
            ...
            case IVEC3:
                if (VEC3_ACCESS.matcher(field).matches()) {
                    return (field.length() == 1 ? PrimitiveType.INT : (field.length() == 2
                                                                       ? PrimitiveType.IVEC2
                                                                       : PrimitiveType.IVEC3));
                }
                break;
    ...
    public Environment validate(Environment environment) {
        ...
        if (base instanceof Struct) {
            ...
            case BVEC2:
                if (!VEC2_ACCESS.matcher(field).matches()) {
                    throw new IllegalStateException(
                            "Illegal swizzle for 2-component vector");
                }
                break;
            case VEC3:
            case IVEC3:
            case BVEC3:
                if (!VEC3_ACCESS.matcher(field).matches()) {
                    throw new IllegalStateException(
                            "Illegal swizzle for 3-component vector");
                }
                break;
            case VEC4:
            case IVEC4:
            case BVEC4:
                if (!VEC4_ACCESS.matcher(field).matches()) {
                    throw new IllegalStateException(
                            "Illegal swizzle for 4-component vector");
                }
                break;
            default:
                throw new IllegalStateException(
                        "Primitive type does not support field access");
            }
        }
