...
/**
 * LwjglAWTSurface is an AWT implementation of OnscreenSurface that uses a
 * GLCanvas as its rendering surface.
 * 
 * @author Michael Ludwig
 */
public class LwjglAWTSurface extends AbstractOnscreenSurface implements WindowListener {
    ...
    public LwjglAWTSurface(AbstractFramework framework, final LwjglSurfaceFactory factory, 
        ...
        Utils.invokeOnAWTThread(new Runnable() {
            @Override
            public void run() {
                OnscreenSurfaceOptions options = LwjglAWTSurface.this.options;
                ...
                if (options.getFullscreenMode() != null) {
                    ...
                    if (device.isFullScreenSupported()) {
                        if (device.isDisplayChangeSupported()) {
                            ...
                            LwjglAWTSurface.this.options = options.setFullscreenMode(factory.getDefaultDisplayMode());
                        }
                    } else {
                        // must not claim fullscreen window anymore
                        LwjglAWTSurface.this.options = options.setFullscreenMode(null);
                    }
                }
            }
        }, true);
        ...
        context = new LWJGLContext(factory, canvas.getContext(), provider);
        vsync = false;
    ...
    public void onSurfaceActivate(OpenGLContext context, int activeLayer) {
        ...
        GL2GL3 gl = ((LWJGLContext) context).getGLContext().getGL().getGL2GL3();
    ...
    private static GLCapabilities chooseCapabilities(LwjglSurfaceFactory factory, OnscreenSurfaceOptions request) {
