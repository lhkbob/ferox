...
public class JoglTextureResourceDriver extends AbstractTextureResourceDriver {
    ...
    protected void glTextureParameters(OpenGLContext context, Texture tex, TextureHandle handle) {
        ...
        if (handle.filter != tex.getFilter()) {
            ...
            gl.glTexParameteri(target, GL2GL3.GL_TEXTURE_MIN_FILTER, min);
            gl.glTexParameteri(target, GL2GL3.GL_TEXTURE_MAG_FILTER, mag);
        }
        
        // wrap s/t/r
        if (handle.wrapS != tex.getWrapModeS()) {
            handle.wrapS = tex.getWrapModeS();
            gl.glTexParameteri(target, GL2GL3.GL_TEXTURE_WRAP_S, getWrapMode(handle.wrapS, caps));
        }
        if (handle.wrapT != tex.getWrapModeT()) {
            handle.wrapT = tex.getWrapModeT();
            gl.glTexParameteri(target, GL2GL3.GL_TEXTURE_WRAP_T, getWrapMode(handle.wrapT, caps));
        }
        if (handle.wrapR != tex.getWrapModeS()) {
            handle.wrapR = tex.getWrapModeS();
            gl.glTexParameteri(target, GL2GL3.GL_TEXTURE_WRAP_R, getWrapMode(handle.wrapR, caps));
        }
        ...
        if (caps.getMaxAnisotropicLevel() > 0) {
            if (handle.anisoLevel != tex.getAnisotropicFilterLevel()) {
                ...
                gl.glTexParameterf(target, GL2GL3.GL_TEXTURE_MAX_ANISOTROPY_EXT, amount);
            }
        }
