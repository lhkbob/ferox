...
public class JoglGlslShaderResourceDriver extends AbstractGlslShaderResourceDriver {
    ...
    protected void updateAttributes(OpenGLContext context, GlslShaderHandle handle) {
        ...
        int[] query = new int[1];
        gl.glGetProgramiv(handle.programID, GL2GL3.GL_ACTIVE_ATTRIBUTES, query, 0);
        int numAttrs = query[0];
        
        gl.glGetProgramiv(handle.programID, GL2GL3.GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, query, 0);
    ...
    protected void updateUniforms(OpenGLContext context, GlslShaderHandle handle) {
        ...
        int[] query = new int[1];
        gl.glGetProgramiv(handle.programID, GL2GL3.GL_ACTIVE_UNIFORMS, query, 0);
        int numUniforms = query[0];
        
        gl.glGetProgramiv(handle.programID, GL2GL3.GL_ACTIVE_UNIFORM_MAX_LENGTH, query, 0);
